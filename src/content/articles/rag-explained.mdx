...
At *T* = 1.0, the distribution follows the model's trained probabilities. At *T* → 0, it collapses — the model always picks the most probable token. At *T* > 1.0, it flattens — the model becomes more random.
...
The mathematics above is correct. The pipeline works as described. But none of it helps if the source material is poor.
...
What organisations are not getting from vendors is a knowledge base that is ready to be indexed. That work precedes deployment — and determines whether the system is useful or embarrassing.
...
Knowledge people should be answering that. Right now, most of them are not in the room.
