...
As a Technical Writer, it's my responsibility to produce release notes for several products (actually 10, with different and frequent release cadences). For as long as the process existed, release notes were produced the same way: open the Azure DevOps board, go through each work item one by one, figure out what it actually meant, and write it up as an accordion entry in a Helpjuice article. Then do it again for the next product. Then the next release.

It worked, in the way that manual processes work before someone stops to measure the cost.

The work itself wasn't difficult, <strong>and that was part of the problem.</strong> It was the kind of task that's easy enough to do but too structured to justify doing by hand indefinitely: pull the same fields, apply the same format, publish to the same place. Repeatable, manual, and just complex enough that it couldn't be handed off without explanation.
Before we knew it, we were spending days drowning in release notes production, with no time to invest in actually governing our processes.

One thing was true: the process needed to be automated.
