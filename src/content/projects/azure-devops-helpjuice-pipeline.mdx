...
Ten products releasing weekly. Each cycle taking upwards of two hours from opening the board to publishing. That's more than half a working week spent on a single repeatable task — and it was time that sat entirely with me.
...
We use PowerAutomate to automatically extract the result set of the Azure query, write it in an excel file, and send it to us via email.

For products that have a fixed release cadence, we know when the board "closes" and at what time we can start producing the release notes.
Let's consider a product whose sprint review happens biweekly on Fridays and ends at 13h. Our PowerAutomate flow would look like this:
...
[3] Run script         — Office Script writes query results to Excel
     │
[5] Apply to each      — iterates over each work item
     │
[6] Send an email      — delivers the Excel file to the team
```
